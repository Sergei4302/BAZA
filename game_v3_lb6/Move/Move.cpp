...
void Move::Movement(Field *field, Player *gamer, Unit **evil, Things **things, int Evil, int Thing,Logger* logger, LoggerImplication* loggerImplication) {
    if(HIGHT +WIDTH < 8){
        throw std::invalid_argument("Incorrect field sizes");
    }
    RenderWindow window(VideoMode(WIDTH*40+120, HIGHT*40), "RealLife");
    Draw *draw =new Draw(field->getCell());
    int* coord;
    bool pressed = false;
    while(window.isOpen()){
        window.clear();
        ...
        while(window.pollEvent(event)){
            ...
            if(static_cast<Key>(event.key.code) == Key::Right&& !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0], coord[1]+1);
            }
            if(static_cast<Key>(event.key.code) == Key::Up && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0]-1, coord[1]);
            }
            if(static_cast<Key>(event.key.code) == Key::Left && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0], coord[1]-1);
            }
            if(static_cast<Key>(event.key.code) == Key::Down && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0]+1, coord[1]);
            }
            if(static_cast<Key>(event.key.code) == Key::Save && !pressed){
                pressed = true;
                SavePlayer* savePlayer=new SavePlayer(gamer,field,logger,loggerImplication);
                SaveOther* saveOther=new SaveOther(field,logger,evil,things,Evil,Thing,draw);
                Save::Savegame(savePlayer,saveOther);
            }
            if(static_cast<Key>(event.key.code) == Key::Load && !pressed){
                pressed = true;
                SavePlayer* savePlayer=new SavePlayer(gamer,field,logger,loggerImplication);
                SaveOther* saveOther=new SaveOther(field,logger,evil,things,Evil,Thing,draw);
...
void Move::MoveEvil(Field* field, Unit** evils, int Evil) {
    ...
    for(int i = 0; i < Evil; i++){
        if(evils[i]->IsAlive()) {
            ...
            evils[i]->Move(field->getCell(), y, x);
        }
    }
}
