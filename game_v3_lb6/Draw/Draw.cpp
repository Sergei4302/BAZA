...
void Draw::DrawFieldOnWindow(RenderWindow *window) {
    for(int i = 0; i < HIGHT; i++){
        for(int j = 0; j < WIDTH; j++){
            cellType = cell[i][j].getType();
            objectType = cell[i][j].GetObjectType();
            ...
            if(objectType == hero){
                ...
                text.setFont(font);
                text.setString("HP: " + std::to_string(cell[i][j].GetObject()->GetHealth()));
                text.setCharacterSize(40);
                text.setColor(Color::Red);
                text.setStyle( Text::StrikeThrough);
                text.setPosition(WIDTH*40 + 5, 10);
                window->draw(text);
                text.setString("Force: " + std::to_string(cell[i][j].GetObject()->GetForce()));
