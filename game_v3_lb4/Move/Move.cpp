...
void Move::Movement(Field *field, Player *gamer, Unit **evil, Things **things, int Evil, int Thing,Logger* logger) {
    ...
    while(window.isOpen()){
        ...
        while(window.pollEvent(event)){
            ...
            if((event.key.code == sf::Keyboard::A ||event.key.code == sf::Keyboard::Left) && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0], coord[1]-1);
            }
            if((event.key.code == sf::Keyboard::W || event.key.code == sf::Keyboard::Up)&& !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0]-1, coord[1]);
            }
            if((event.key.code == sf::Keyboard::D || event.key.code == sf::Keyboard::Right) && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0], coord[1]+1);
            }
            if((event.key.code == sf::Keyboard::S ||event.key.code == sf::Keyboard::Down) && !pressed){
                pressed = true;
                MoveHero(field, gamer, coord[0]+1, coord[1]);
            }
